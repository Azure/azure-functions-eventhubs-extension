...
namespace Microsoft.Azure.WebJobs.Host.EndToEndTests
{
    public class EventHubEndToEndTests
    {
        ...
        public async Task EventHub_PartitionKeySender_Single(Type type)
        {
            using (JobHost host = BuildHost<EventHubParitionKeyTestJobs>())
            {
                var method = typeof(EventHubParitionKeyTestJobs).GetMethod("SendEvents_PartitionSenderSingle", BindingFlags.Static | BindingFlags.Public);
                _eventWait = new ManualResetEvent(initialState: false);
                switch (type)
                {
        ...
        [InlineData(typeof(EventData))]
        [InlineData(typeof(DummyClass))]
        public async Task EventHub_PartitionKeySender_Multiple(Type type)
        {
            using (JobHost host = BuildHost<EventHubParitionKeyTestJobs>())
            {
                var method = typeof(EventHubParitionKeyTestJobs).GetMethod("SendEvents_PartitionSenderMultiple", BindingFlags.Static | BindingFlags.Public);
                _eventWait = new ManualResetEvent(initialState: false);
                switch (type)
                {
                    case Type stringType when stringType == typeof(string):
        ...
        {
            public static async Task SendEvents_PartitionSenderMultiple(
                object input,
                [EventHub(TestHubName)] IAsyncCollector<PartitionedValue> events)
            {
            ...
            public static void SendEvents_PartitionSenderSingle(
                object input,
                [EventHub(TestHubName)] out PartitionedValue data)
            {
                string parttionKey = "test_pk_single";
                data = null;
                switch (input.GetType())
                {
                    case Type stringType when stringType == typeof(string):
                        data = new PartitionedValue(input as string, parttionKey);
                        break;
                    case Type byteArrayType when byteArrayType == typeof(byte[]):
                        data = new PartitionedValue(input as byte[], parttionKey);
                        break;
                    case Type eventDataType when eventDataType == typeof(EventData):
                        data = new PartitionedValue(input as EventData, parttionKey);
                        break;
                    case Type classType when classType == typeof(DummyClass):
                        data = new PartitionedValue(input as DummyClass, parttionKey);
                        break;
                }
            }

            private static async Task AddWithPartitionKeyAsync(object input, string parttionKey, IAsyncCollector<PartitionedValue> events)
            {
                switch (input.GetType())
                {
                    case Type stringType when stringType == typeof(string):
                        await events.AddAsync(new PartitionedValue(input as string, parttionKey));
                        break;
                    case Type byteArrayType when byteArrayType == typeof(byte[]):
                        await events.AddAsync(new PartitionedValue(input as byte[], parttionKey));
                        break;
                    case Type eventDataType when eventDataType == typeof(EventData):
                        await events.AddAsync(new PartitionedValue(input as EventData, parttionKey));
                        break;
                    case Type classType when classType == typeof(DummyClass):
                        await events.AddAsync(new PartitionedValue(input as DummyClass, parttionKey));
                        break;
                }
            }
            ...
            public static void ProcessMultiplePartitionEvents([EventHubTrigger(TestHubName)] EventData[] events)
            {
                foreach (EventData eventData in events)
                {
                    ...
                    if (message.Contains(_testId))
                    {
                        ...
                        if ((_results.Count == 6 && _results[5] == "test_pk4") || _results[0] == "test_pk_single")
                        {
                            _eventWait.Set();
                        }
                    }
